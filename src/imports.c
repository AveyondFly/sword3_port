...
/* TTF 渲染 wrapper（libbionic_shim.c 定义，带 GBK/BIG5->UTF-8 转码）。
 * 声明于此，供 my_dlsym（运行时 dlsym 拦截）与 dynlib_functions[]（GOT/PLT 强制）
 * 两处引用，覆盖"运行时 dlsym(libSDL2_ttf,...) 绕过 LD_PRELOAD"的路径。 */
...
// glDrawTexfOES é uma extensão GLES OES que NÃO é garantida pelo libGLESv2 do
// toolchain em link estático (o Summertime Saga original dependia do libGLESv2
// da toolchain LibreELEC exportá-la; no docker build ela não existe). Resolvemos
// em runtime via dlsym(RTLD_DEFAULT) e encaminhamos; se indisponível, vira no-op.
// Jogos 2D SDL2 (ex.: sword3) em geral nunca chamam esta função.
typedef void (*PFNGLDRAWTEXFOESPROC)(GLfloat, GLfloat, GLfloat, GLfloat,
                                     GLfloat);
void glDrawTexfOES(GLfloat x, GLfloat y, GLfloat z, GLfloat width,
                   GLfloat height) {
  static PFNGLDRAWTEXFOESPROC fn = (PFNGLDRAWTEXFOESPROC)-1;
  if (fn == (PFNGLDRAWTEXFOESPROC)-1) {
    fn = (PFNGLDRAWTEXFOESPROC)dlsym(RTLD_DEFAULT, "glDrawTexfOES");
  }
  if (fn) {
    fn(x, y, z, width, height);
  }
}
...
static void *my_dlsym(void *handle, const char *name) {
  ...
  /* TTF 渲染拦截：游戏运行时 dlsym(libSDL2_ttf, "TTF_RenderUTF8_*") 会绕过
   * LD_PRELOAD 与 so_resolve 的 GOT 填充，直接拿到真实函数。在此统一返回带
   * GBK/BIG5->UTF-8 转码的 wrapper，确保文本渲染入口必过 shim。
   * 注：wrapper 内部经 dlsym(RTLD_NEXT) 取真函数（main 二进制自身 dlsym，
   * 不经本 my_dlsym），无递归风险。 */
  if (name) {
...
DynLibFunction dynlib_functions[] = {
    ...
    /* TTF 渲染强制走 shim wrapper（GBK/BIG5->UTF-8 转码，修复"不显示文字"） */
